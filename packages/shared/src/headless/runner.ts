import { LinkAgent, type LinkAgentConfig, type PermissionMode, type SdkMcpServerConfig } from '../agent/link-agent.ts';
...
export class HeadlessRunner {
  ...
  private agent: LinkAgent | null = null;
  ...
  async *runStreaming(): AsyncGenerator<HeadlessEvent> {
    try {
      ...
      // 2. Create LinkAgent with headless callbacks
      this.createAgent();
...
${this.config.prompt}
  ...
  /**
   * Create LinkAgent with headless callbacks for permissions and questions.
   */
  private createAgent(): void {
    ...
    const agentConfig: LinkAgentConfig = {
    ...
    this.agent = new LinkAgent(agentConfig);
