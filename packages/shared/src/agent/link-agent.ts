...
export interface LinkAgentConfig {
  workspace: Workspace;
...
export class LinkAgent {
  private config: LinkAgentConfig;
  ...
  constructor(config: LinkAgentConfig) {
    ...
    registerSessionScopedToolCallbacks(sessionId, {
      onPlanSubmitted: (planPath) => {
        this.onDebug?.(`[LinkAgent] onPlanSubmitted received: ${planPath}`);
        this.onPlanSubmitted?.(planPath);
      },
      onAuthRequest: (request) => {
        this.onDebug?.(`[LinkAgent] onAuthRequest received: ${request.sourceSlug} (type: ${request.type})`);
  ...
  private startConfigWatcher(): void {
    ...
    this.configWatcher = createConfigWatcher(this.workspaceRootPath, {
      onSourceChange: (slug, source) => {
        debug('[LinkAgent] Source changed:', slug, source ? 'updated' : 'deleted');
        this.onSourceChange?.(slug, source);
      },
      onSourcesListChange: (sources) => {
        debug('[LinkAgent] Sources list changed:', sources.length);
        this.onSourcesListChange?.(sources);
      },
      onValidationError: (file, result) => {
        debug('[LinkAgent] Config validation error:', file, result.errors);
        this.onConfigValidationError?.(file, result.errors);
      },
      onError: (file, error) => {
        debug('[LinkAgent] Config file error:', file, error.message);
      },
    });

    debug('[LinkAgent] Config watcher started');
  }
  ...
  private stopConfigWatcher(): void {
    if (this.configWatcher) {
      ...
      debug('[LinkAgent] Config watcher stopped');
    }
  }
  ...
  private handleSourceUpdate(slug: string, source: LoadedSource | null): void {
    if (!source) {
      ...
      debug('[LinkAgent] Removed source:', slug);
      return;
    }
  ...
  setThinkingLevel(level: ThinkingLevel): void {
    ...
    this.onDebug?.(`[LinkAgent] Thinking level: ${level}`);
  }
  ...
  setUltrathinkOverride(enabled: boolean): void {
    ...
    this.onDebug?.(`[LinkAgent] Ultrathink override: ${enabled ? 'ENABLED' : 'disabled'}`);
  }
  ...
  async *chat(
    ...
    try {
        ...
        hooks: {
          ...
          SubagentStart: [{
            hooks: [async (input, _hookToolUseID) => {
              ...
              console.log(`[LinkAgent] SubagentStart: agent_id=${typedInput.agent_id}, type=${typedInput.agent_type}`);
              return { continue: true };
            }],
          }],
          SubagentStop: [{
            hooks: [async (input, _toolUseID) => {
              ...
              console.log(`[LinkAgent] SubagentStop: agent_id=${typedInput.agent_id}`);
              return { continue: true };
            }],
          }],
        },
      ...
      if (wasResuming) {
        console.error(`[LinkAgent] Attempting to resume SDK session: ${this.sessionId}`);
        debug(`[LinkAgent] Attempting to resume SDK session: ${this.sessionId}`);
      } else {
        console.error(`[LinkAgent] Starting fresh SDK session (no resume)`);
        debug(`[LinkAgent] Starting fresh SDK session (no resume)`);
      }
      ...
      try {
        ...
        console.error(`[LinkAgent] INNER CATCH triggered: ${sdkError instanceof Error ? sdkError.message : String(sdkError)}`);
        ...
        if (isProcessError) {
          ...
          if (isSessionExpired && wasResuming && !_isRetry) {
            ...
            console.error('[LinkAgent] SDK session expired server-side, clearing and retrying fresh');
            debug('[LinkAgent] SDK session expired server-side, clearing and retrying fresh');
            this.sessionId = null;
      ...
      console.error(`[LinkAgent] OUTER CATCH triggered: ${error instanceof Error ? error.message : String(error)}`);
      console.error(`[LinkAgent] Error stack: ${error instanceof Error ? error.stack : 'no stack'}`);
  ...
  private convertSDKMessage(
    ...
    switch (message.type) {
          ...
          if (block.type === 'text') {
            ...
            if (!emittedToolStarts.has(block.id)) {
              ...
              if (isParentTool) {
                ...
                console.log(`[LinkAgent] CHILD REGISTERED (assistant/single-parent): ${block.name} (${block.id}) under parent ${parentId}`);
              } else if (activeParentTools.size > 1) {
                ...
                if (sdkParentId && activeParentTools.has(sdkParentId)) {
                  ...
                  console.log(`[LinkAgent] CHILD REGISTERED (assistant/sdk-parent): ${block.name} (${block.id}) → Task (${sdkParentId})`);
                } else {
                  ...
                  childToParent.set(block.id, parentId);
                  parentToolUseId = parentId;
                  console.log(`[LinkAgent] CHILD REGISTERED (assistant/fifo-fallback): ${block.name} (${block.id}) → ${parentId} (sdk_parent=${sdkParentId})`);
                }
              }
      ...
      case 'stream_event': {
        ...
        if (event.type === 'content_block_delta' && event.delta.type === 'text_delta') {
          ...
          if (!emittedToolStarts.has(toolBlock.id)) {
            ...
            console.log(`[LinkAgent] TOOL START: ${toolBlock.name} (${toolBlock.id}), sdk_parent=${sdkParentId}, activeParents=[${Array.from(activeParentTools).join(',')}]`);
            ...
            if (isParentTool) {
              ...
              console.log(`[LinkAgent] PARENT REGISTERED (stream): ${toolBlock.name} (${toolBlock.id})`);
              ...
              console.log(`[LinkAgent] CHILD REGISTERED (stream/sdk): ${toolBlock.name} (${toolBlock.id}) under parent ${sdkParentId}`);
            } else if (activeParentTools.size === 1) {
              // Single active parent - unambiguous, assign directly
              const parentId = Array.from(activeParentTools)[0]!;
              ...
              parentToolUseId = parentId;
              console.log(`[LinkAgent] CHILD REGISTERED (stream/single-parent): ${toolBlock.name} (${toolBlock.id}) under parent ${parentId}`);
            } else if (activeParentTools.size > 1) {
              // Multiple active parents - use SDK's parent_tool_use_id (authoritative source)
              ...
              if (sdkParentId && activeParentTools.has(sdkParentId)) {
                ...
                childToParent.set(toolBlock.id, sdkParentId);
                parentToolUseId = sdkParentId;
                console.log(`[LinkAgent] CHILD REGISTERED (stream/sdk-parent): ${toolBlock.name} (${toolBlock.id}) → Task (${sdkParentId})`);
              } else {
                // Fallback: FIFO if SDK doesn't provide parent
                const parentId = Array.from(activeParentTools)[0]!;
                parentToChildren.get(parentId)?.push(toolBlock.id);
                childToParent.set(toolBlock.id, parentId);
                parentToolUseId = parentId;
                console.log(`[LinkAgent] CHILD REGISTERED (stream/fifo-fallback): ${toolBlock.name} (${toolBlock.id}) → ${parentId} (sdk_parent=${sdkParentId})`);
              }
            }
            // else: no active parents - tool is top-level, no parent needed
      ...
      case 'user': {
        ...
        if (message.tool_use_result !== undefined) {
          ...
          if (toolUseId && activeParentTools.has(toolUseId)) {
            ...
            console.log(`[LinkAgent] RESULT MATCHING: parent=${toolUseId}, children.length=${children?.length || 0}`);
            if (children && children.length > 0) {
              const firstChild = children.shift()!; // Remove first child (FIFO)
              console.log(`[LinkAgent] MATCHED TO CHILD: ${firstChild}`);
              ...
              if (pendingChildId) {
                ...
                console.log(`[LinkAgent] MATCHED TO LATE CHILD: ${pendingChildId} (parent=${toolUseId})`);
                ...
                console.log(`[LinkAgent] NO CHILDREN LEFT - treating as parent's own result: ${toolUseId}`);
            ...
            if (pendingChildId) {
              ...
              console.log(`[LinkAgent] MATCHED TO ORPHANED CHILD: ${pendingChildId} (parent=${toolUseId})`);
      ...
      case 'tool_progress': {
        ...
        console.log(`[LinkAgent] tool_progress: tool=${progress.tool_name} (${progress.tool_use_id}), parent=${progress.parent_tool_use_id}, elapsed=${progress.elapsed_time_seconds}`);
        ...
        if (!emittedToolStarts.has(progress.tool_use_id)) {
          ...
          if (!isParentTool && progress.parent_tool_use_id && activeParentTools.has(progress.parent_tool_use_id)) {
            ...
            console.log(`[LinkAgent] CHILD REGISTERED (tool_progress/sdk): ${progress.tool_name} (${progress.tool_use_id}) → ${progress.parent_tool_use_id}`);
          }
      ...
      case 'result': {
        ...
        console.error(`[LinkAgent] result message: subtype=${message.subtype}, errors=${'errors' in message ? JSON.stringify((message as any).errors) : 'none'}`);
