 ...
 * Deep Link Handler
 *
 * Parses linkagents:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   linkagents://allChats[/chat/{sessionId}]            - Chat list (all chats)
 *   linkagents://flagged[/chat/{sessionId}]             - Chat list (flagged filter)
 *   linkagents://state/{stateId}[/chat/{sessionId}]     - Chat list (state filter)
 *   linkagents://sources[/source/{sourceSlug}]          - Sources list
 *   linkagents://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   linkagents://action/{actionName}[/{id}][?params]
 *   linkagents://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   linkagents://allChats/chat/abc123                   (specific chat)
 *   linkagents://settings/shortcuts                     (shortcuts page)
 *   linkagents://sources/source/github                  (github source info)
 *   linkagents://action/new-chat                        (uses active window)
 *   linkagents://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   linkagents://workspace/ws123/allChats/chat/abc123   (targets specific workspace)
 */
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    // e.g., linkagents://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., linkagents://allChats/chat/abc → hostname='allChats', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // linkagents://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // linkagents://allChats/..., linkagents://settings/..., etc. (compound routes)
    ...
    // linkagents://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // linkagents://action/... (no workspace - uses active window)
    if (host === 'action') {
